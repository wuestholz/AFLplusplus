...
havoc_stage:
  ...
  r_max = 89 + (afl->extras_cnt ? 8 : 0) + (afl->a_extras_cnt ? 2 : 0);
...
#ifdef INTROSPECTION
            snprintf(afl->m_tmp, sizeof(afl->m_tmp), " CLONE-%u-%u-%u",
                     clone_from, clone_to, clone_len);
            strcat(afl->mutation, afl->m_tmp);
#endif
            new_buf =
...
#ifdef INTROSPECTION
            snprintf(afl->m_tmp, sizeof(afl->m_tmp), " INSERT-%u-%u-%u",
                     clone_from, clone_to, clone_len);
            strcat(afl->mutation, afl->m_tmp);
#endif
            new_buf =
        ...
        default:
          ...
          if (afl->a_extras_cnt) {
            ...
            if (r < 1) {
              ...
              u32 use_a_extra = rand_below(afl, afl->a_extras_cnt);
              u32 a_extra_len = afl->a_extras[use_a_extra].len;

              if (a_extra_len > temp_len) { break; }

              u32 insert_at = rand_below(afl, temp_len - a_extra_len + 1);
#ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp),
                       " AUTO_EXTRA_OVERWRITE-%u-%u", insert_at, a_extra_len);
              strcat(afl->mutation, afl->m_tmp);
#endif
              memcpy(out_buf + insert_at, afl->a_extras[use_a_extra].data,
                     a_extra_len);
              ...
              u32 use_a_extra = rand_below(afl, afl->a_extras_cnt);
              u32 a_extra_len = afl->a_extras[use_a_extra].len;
              u8 *ptr = afl->a_extras[use_a_extra].data;
#ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp),
                       " AUTO_EXTRA_INSERT-%u-%u", insert_at, a_extra_len);
              strcat(afl->mutation, afl->m_tmp);
#endif
              if (temp_len + a_extra_len >= MAX_FILE) { break; }

              out_buf = afl_realloc(AFL_BUF_PARAM(out), temp_len + a_extra_len);
              if (unlikely(!out_buf)) { PFATAL("alloc"); }

              u32 insert_at = rand_below(afl, temp_len + 1);
              /* Tail */
              memmove(out_buf + insert_at + a_extra_len, out_buf + insert_at,
                      temp_len - insert_at);

              /* Inserted part */
              memcpy(out_buf + insert_at, ptr, a_extra_len);

              temp_len += a_extra_len;
